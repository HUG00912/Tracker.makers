...
function hostDe(url) { try { return new URL(url).hostname; } catch (e) { return ""; } }
  ...
  function trackerView() {
    ...
    const nav = (id, ic, label, badge, corto, cls) => `
      <button class="nav ${cls || ""} ${(state.mainView === id || (id === "mas" && ["portal", "eliminados"].includes(state.mainView))) ? "on" : ""}" data-main="${id}">
        ${icon(ic, 16)}<span class="grow"><span class="lg">${label}</span><span class="sm">${corto || label}</span></span>
    ...
    const railEquipo = `
      ...
      ${nav("pendientes", "clock", "Pendientes", totalPend || null, null, "only-m")}
      ${nav("portal", "home", "Portal del cliente", null, null, "no-m")}
      <div class="grp">GESTIÓN</div>
      ${nav("clientes", "users2", "Clientes")}
      ${nav("eliminados", "close", "Eliminados", null, null, "no-m")}
      ${nav("mas", "more", "Más", null, null, "only-m")}
      ${state.mainView === "pendientes" ? "" : `<div class="rail-foot">
    ...
    const railCliente = `
      ...
      ${nav("proyectos", "board", "Mis proyectos", pendientesClienteCount(clientVideos) || null, "Proyectos")}
  ...
  function mainEquipo(teamVideos, clientVideos, nombreCliente) {
    ...
    if (state.mainView === "mas") {
      return `<div class="page">
        <div class="ph"><div><h1>Más</h1><div class="desc">Otras secciones del tracker.</div></div></div>
        <button class="btn block mas-item" data-main="portal">${icon("home", 18)}<span><b>Portal del cliente</b><small>Así ve el cliente su cuenta (vista previa)</small></span></button>
        <button class="btn block mas-item" data-main="eliminados">${icon("close", 18)}<span><b>Eliminados</b><small>Registro de proyectos eliminados</small></span></button>
      </div>`;
    }
    if (state.mainView === "pendientes") {
      return `<div class="page pend-page">
        <div class="ph"><div><h1>Pendientes</h1><div class="desc">Lo de hoy, las novedades y lo que viene este mes.</div></div></div>
        <div class="panel">${pendientesRail()}</div>
      </div>`;
    }
